...
<div x-data="{ open: false, tab: 'profil' }" class="px-8 py-8 bg-[#f8fafc] min-h-screen space-y-8 animate-fade-in">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button @click="open = true" class="group flex items-center gap-2 px-5 py-3.5 bg-blue-600 text-white font-bold rounded-2xl shadow-xl shadow-blue-200 hover:bg-blue-700 transition-all active:scale-95">
    ...
    <div x-show="open" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md" x-cloak>
        <div @click.outside="open = false" class="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden animate-slide-up">
            ...
            {{-- PERUBAHAN DISINI: diarahkan ke nama route yang sesuai (.update) dengan spoofing method PUT --}}
            <form action="{{ route('admin.organisasi.update') }}" method="POST" class="p-10 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div x-show="tab === 'profil'" class="grid grid-cols-2 gap-6 animate-fade-in">
                    <div class="col-span-2 space-y-2">
                        ...
                        <input type="text" name="nama" value="{{ $organisasi['nama'] ?? '' }}" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-blue-500 transition-all">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Perusahaan Naungan</label>
                        <input type="text" name="perusahaan" value="{{ $organisasi['perusahaan'] ?? '' }}" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-blue-500 transition-all">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Email CS</label>
                        <input type="email" name="email" value="{{ $organisasi['email'] ?? '' }}" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-blue-500 transition-all">
                    </div>
                    <div class="col-span-2 space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Alamat Kantor</label>
                        <textarea name="alamat" rows="2" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-blue-500 transition-all">{{ $organisasi['alamat'] ?? '' }}</textarea>
                    </div>
                </div>
                ...
                <div x-show="tab === 'prosedur'" class="space-y-8 animate-fade-in">
                    ...
                    <div class="grid grid-cols-2 gap-6 pt-8 border-t border-slate-50">
                        ...
                        <div class="col-span-2 space-y-2">
                            ...
                            <select name="payment_method" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-blue-500 transition-all appearance-none cursor-pointer">
                ...
                <div class="flex items-center gap-4 pt-6">
                    <button type="button" @click="open = false" class="flex-1 py-4 text-slate-400 font-bold uppercase text-xs tracking-widest bg-slate-50 rounded-2xl hover:bg-slate-100 transition-colors">Batal</button>
                    <button type="submit" class="flex-[2] py-5 bg-blue-600 text-white font-black rounded-2xl shadow-xl shadow-blue-200 hover:bg-blue-700 transition-all active:scale-95">Simpan & Terapkan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
