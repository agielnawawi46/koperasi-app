    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-3">
                ...
                <div class="p-10 space-y-10">
                    <div class="flex flex-col md:flex-row items-center gap-8 border-b border-slate-50 pb-8">
                        ...
                        <div class="text-center md:text-left">
                            ...
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">{{ $authUser->roles->first()->name ?? 'Admin' }}</p>
                        </div>
                    </div>
            ...
            <div x-show="tab === 'keamanan'" class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-10 space-y-8" x-cloak>
                ...
                <form method="POST" action="{{ route('admin.pengaturan.password') }}" class="max-w-md space-y-6">
                    @csrf
                    @method('PUT')
...
@push('modals')
    {{-- ================= MODAL EDIT PROFIL ================= --}}
    <div id="modalEditProfil" class="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-all duration-300 hidden">
        <div class="bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden transform transition-all duration-300 scale-95 translate-y-8">
            ...
            <div class="p-10">
                <form method="POST" action="{{ route('admin.pengaturan.profile') }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    <div class="space-y-1.5">
